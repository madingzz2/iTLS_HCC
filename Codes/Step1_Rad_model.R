...
icc <- data.frame(matrix(ncol=0,nrow = 3045)) #feature count
modal <- c('AP','T2WI','PVP') 

for (i in modal) {
  df_icc_ori <- fread(paste0('icc/ori_',i,'.csv')) %>% 
    as.data.frame() %>% 
    tibble::column_to_rownames('ID')
  n <- dim(df_icc_ori)[1]
  p <- dim(df_icc_ori)[2]
  
  df <- fread(paste0('icc/aft_',i,'.csv')) %>% 
    as.data.frame() %>% 
    filter(ID %in% all_of(rownames(df_icc_ori))) %>% 
    tibble::column_to_rownames('ID') %>% 
    rbind(df_icc_ori)
  
  result <- apply(df, 2, function(x) ICC(x = data.frame(x[1:n],x[(n+1):(2*n)]),lmer=F)$results[1,2])
  names(result) <- gsub(pattern = paste0('_',i),replacement = '',x = names(result))
  rownames(icc) <- names(result)
  icc <- cbind(icc,result)
}
colnames(icc) <- modal
fwrite(icc,file = 'icc/icc_results.csv',row.names=T) 
#--- z-score----
rm(list = ls())
library(caret)
library(dplyr)

train <- read.csv('radiomic/xyh_Rad_features.csv')
val1 <- read.csv('radiomic/hainan_Rad_features.csv')
val2 <- read.csv('radiomic/mhh_Rad_features.csv')
val3 <- read.csv('radiomic/tcia_Rad_features.csv')
...
write.csv(df_center1, row.names = FALSE,file="radiomic/xyyy_Rad_features_z.csv")
write.csv(df_center2, row.names = FALSE,file="radiomic/hainan_Rad_features_z.csv")
write.csv(df_center3, row.names = FALSE,file="radiomic/mhh_Rad_features_z.csv")
write.csv(df_center4, row.names = FALSE,file="radiomic/tcia_Rad_features_z.csv")
...
modal <- c('AP','T2WI','PVP') 
for (i in modal) {
  icc<- read.csv('icc/icc_results.csv') %>% 
    filter(i>0.75) %>% 
    rownames() 
  icc <- paste0(icc,'_',i)
  icc_features <- c(icc,icc_features)
}


df <- read.csv("radiomic/xyyy_Rad_features_z.csv") %>% 
  select(all_of(icc_features))
df1 <- rbind(
  read.csv("radiomic/hainan_Rad_features_z.csv"),
  read.csv("radiomic/mhh_Rad_features_z.csv")) %>% 
  select(all_of(icc_features))
df2 <- read.csv("radiomic/tcia_Rad_features_z.csv")%>% 
  select(all_of(icc_features))
...
if(T){
  ...
  lasso_selection <- cv.glmnet(x=cv_x,
                               ...
                               alpha = 1,
                               nfolds = 5)
  pdf(file='Rad_lasso1.pdf',width = 6,height = 10)
  ...
  pdf(file='Rad_lasso2.pdf',width = 6,height = 10)
  ...
  write.csv(radscore_train_data2,file = "select feature/train Rad features.csv",row.names = FALSE)
  ...
  write.csv(radscore_valid_data2,file = "select feature/test Rad features.csv",row.names = FALSE)
  ...
  write.csv(radscore_test1_data2,file = "select feature/external Rad features.csv",
            row.names = FALSE)
  ...
  write.csv(radscore_test2_data2,file = "select feature/TCIA Rad features.csv",
            row.names = FALSE)
}
####  Rad model save ####
...
rm(list=ls())
library(pROC)
train <- read.csv("select feature/train Rad features.csv",row.names = "ID")
val <- read.csv("select feature/test Rad features.csv",row.names = "ID")
model <- glm(Label ~ ., data = train,family='binomial')
saveRDS(model,file = 'Rad_model.rds')
summary(model)
#fwrite(data.frame(model$coefficients),file = 'Rad_coefficients.csv',row.names = T)
